...
export class HuntScreen extends BaseScreen {
    ...
    private combatPostCombatReport(report: { enemy: EnemyCharacter; area: Area; xp: number; loot: string[]; renown: number }) {
        this.updateOutput(`You have defeated <span class="rarity-${report.enemy.spec.rarity}"> ${report.enemy.name}</span> and gained ${report.xp} XP and ${report.renown} renown!`);
    }
