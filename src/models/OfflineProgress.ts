...
export class OfflineProgressManager {
    ...
    public processOfflineSession(session: OfflineSession) {
        ...
        printLog(`Processing Offline Session: Offline for ${formatTime(session.duration)}, reason: ${session.reason}`, 3, "OfflineProgress.ts", "offline");
...
class OfflineTreasureCalculator {
    ...
    calculateTreasureRewards(offlineSeconds: number): OfflineTreasureReward {
        ...
        const nextInterval = intervalIndex < this.chestIntervals.length ? this.chestIntervals[intervalIndex] - remainingTime : this.chestIntervals[this.chestIntervals.length - 1] - remainingTime;
